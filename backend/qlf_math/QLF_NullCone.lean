...
def is_null_spectral (s : TopoString) : Prop :=
  ∃ c : ℂ,
    toSpectralMode s = c • (1 : Matrix (Fin 2) (Fin 2) ℂ)

/--
The (0,0) entry of the spectral mode equals the positive-phase count.
-/
lemma toSpectralMode_zero_zero (s : TopoString) :
    (toSpectralMode s) 0 0 = (count_pos s : ℂ) := by
  induction s with
  | nil =>
      rfl
  | cons hd tl ih =>
...
lemma pure_zero_count_implies_empty (s : TopoString)
  ...
  | cons hd tl =>
      have ⟨p, hp_eq⟩ :=
        h_pure hd (List.mem_cons_self hd tl)
      cases p
      · subst hp_eq
...
/--
ZFA implies a nontrivial null-spectral state for every
nonempty pure-phase QLF string.
-/
theorem zfa_implies_null_spectral (s : TopoString)
  ...
  have h_sym : is_symmetric s :=
    zfa_implies_critical_line s h_zfa

  have h_scalar :=
    spectral_symmetric_eq_scalar_id s h_pure h_sym
  ...
  · intro hc_zero
    ...
    have h_eval :
        (toSpectralMode s) 0 0 =
          (c • (1 : Matrix (Fin 2) (Fin 2) ℂ)) 0 0 := by
      rw [hc_eq]
    ...
    have h_pos_zero : count_pos s = 0 := by
    ...
    have h_empty :=
      pure_zero_count_implies_empty
        s h_pure h_pos_zero h_neg_zero
